...
class MusicBot(discord.Client):
    ...
    async def cmd_request(self, channel, author, role):
        """
        Usage:
            {command_prefix}add[role]
        """
        all_roles = channel.server.roles
        role = best_match.decide_role(role, all_roles)

        if role is None:
            return Response("I appreciate your creativity but I can't assign you something that doesn't exist"
                            , delete_after=20)

        if role in constants.NON_ASSIGNABLE_ROLES:
            return Response("Don't tryna be assigning yourself admin privilege you specky cunt"
                            , delete_after=20)
        ...
        return Response("Added role %s" % role + " for %s" % author, delete_after=20)

    async def cmd_remove(self, channel, author, role):
        """
        Usage:
            {command_prefix}remove[role]
        """
        all_roles = channel.server.roles
        role = best_match.decide_role(role, all_roles)
        ...
        return Response("Removed role %s" % role + " for %s" % author, delete_after=20)

    async def cmd_roll(self):
        """
        For all your gacha needs
        :return: 
        """
        draw_rate_file = "D:\Musicbot\config\draw_rate.txt"

        response = '__Draw Result__\n'

        for i in range(0, 10):
            random_roll = random.uniform(0.0, 100.0)

            # Type conversion bullshit
            if random_roll > 17.9:
                response += "*" + "Useless Rare Shit" + "*\n"
                continue

            with open(draw_rate_file) as f:
                current_sum = 0.0
                prev_name = ''
                # May be slow theoretically but fuck it
                for num, line in enumerate(f, 1):
                    name, rate = line.split('=')
                    rate = float(rate.strip())
                    if current_sum + rate > random_roll:
                        type = draw_type.draw_type_of(current_sum)
                        letter_type = draw_type.letter_type(type)

                        response += letter_type + type + " " + prev_name + letter_type + "\n"
                        break
                    current_sum += rate
                    prev_name = name
    ...
    async def cmd_hasagni(self):
        """
        Cuz fuck it why not
        :return: 
        """
        return Response("REEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"
                        "EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE")

    async def cmd_gifadd(self, name, link):
        """
        Usage:
            {command_prefix}[name][link]
        Because I'm too lazy to do this for you
        """
        if "http://" not in link and "https://" not in link:
            link = "https://" + link

        if os.path.isfile(constants.GIF_PATH + name + '.gif'):
            return Response("Oi, it seems this already exists. don't be tryna overwrite shit you hooligan",
                            reply=True, delete_after=20)

        else:
            try:
                with open(constants.GIF_PATH + name + '.gif', 'wb') as f:
                    resp = request.urlopen(link)
                    f.write(resp.read())
                    return Response("I guess it worked, try it out")
            except urllib.request.URLError as err:
                os.remove(constants.GIF_PATH + name + '.gif')
                return Response("I can't download this, but here's the error: " +
                                str(err.reason) + ".Please forgib :(", reply=True, delete_after=20)

    async def cmd_gifedit(self, name, link):
        """
        Usage:
            {command_prefix}[name][link]
        Because I'm too lazy to do this for you
        ...
        if not os.path.isfile(constants.GIF_PATH + name + '.gif'):
            return Response("If you want to edit something, at least spell the thing right",
                            reply=True, delete_after=20)
        else:
            # Remove old file
            os.remove(constants.GIF_PATH + name + '.gif')
    ...
    async def cmd_gif(self, gif=None, channel='catalogue'):
        """
        Usage:
            {command_prefx}emoji [emoji]
        Spam chat with random shit
        I still can't believe no one bothered to do this before.
        """
        try_match = best_match.decide_match(gif, ".gif")

        if try_match is not None:
            gif = try_match

        try:
            r = constants.GIF_PATH + gif + ".gif"
            await self.send_file(channel, r)
        except FileNotFoundError:
            return Response("oi mate, that gif doesnt exist or you mispelled it or something", reply=True,
                            delete_after=20)
        return None

    async def cmd_emojiadd(self, name, link):
        """
        Usage:
            {command_prefix}[name][link]
        Because I'm too lazy to do this for you
        ...
        randomable_download_dir = constants.IMAGE_PATH + name + '/'
        normal_download_dir = constants.IMAGE_PATH

        if os.path.isfile(constants.IMAGE_PATH + name + '.png') and not os.path.isdir(randomable_download_dir):
            return Response("Oi, it seems this already exists. don't be tryna overwrite shit you hooligan",
                            reply=True, delete_after=20)

        # If requested into randomable folders
        if os.path.isdir(randomable_download_dir):
            random_image_name = str(len(os.listdir(constants.IMAGE_PATH + name)))
            try:
                with open(randomable_download_dir + random_image_name + '.png', 'wb') as f:
                    resp = request.urlopen(link)
                    f.write(resp.read())
                    return Response("I guess it worked, try it out")
            except urllib.request.URLError as err:
                os.remove(randomable_download_dir + random_image_name + '.png')
                return Response("I can't download this, but here's the error: " +
                                str(err.reason) + ".Please forgib :(", reply=True, delete_after=20)

        # If requested into normal imgs folder
        else:
            try:
                with open(normal_download_dir + name + '.png', 'wb') as f:
                    resp = request.urlopen(link)
                    f.write(resp.read())
                    return Response("I guess it worked, try it out")
            except urllib.request.URLError as err:
                os.remove(normal_download_dir + name + '.png')
                return Response("I can't download this, but here's the error: " +
                                str(err.reason) + ".Please forgib :(", reply=True, delete_after=20)

    async def cmd_emojiedit(self, name, link):
        """
        Usage:
            {command_prefix}[name][link]
        Because I'm too lazy to do this for you
        ...
        if not os.path.isfile(constants.IMAGE_PATH + name + '.png'):
            return Response("If you want to edit something, at least spell the thing right",
                            reply=True, delete_after=20)
        else:
            # Remove old file
            os.remove(constants.IMAGE_PATH + name + '.png')
    ...
    async def cmd_emojiaddgroup(self, group_name):
        """
        Usage:
            {command_prefix}[group_name][link]
        :param group_name: 
        :return: 
        """
        requested_group = constants.IMAGE_PATH + group_name

        if not os.path.exists(requested_group):
            os.makedirs(requested_group)
            return Response("It is done, try putting in some images", reply=True, delete_after=20)
        return Response("Oi, it seems this already exists. don't be tryna overwrite shit you hooligan",
                        reply=True, delete_after=20)

    async def cmd_emo(self, emoji=None, channel='catalogue'):
        """
        Usage:
            {command_prefx}emoji [emoji]
        Spam chat with random shit
        I still can't believe no one bothered to do this before.
        """
        try_match = best_match.decide_match(emoji, ".png")

        if try_match is not None:
            emoji = try_match

        try:
            # Check if image is in randomable list
            if auto_gen.is_randomable(emoji):
                generate_image = auto_gen.folder(emoji)
                auto_gen.random_tracker[emoji] = generate_image
                r = constants.IMAGE_PATH + emoji + "/" + generate_image + ".png"
            else:
                r = constants.IMAGE_PATH + emoji + ".png"

            await self.send_file(channel,r)

        except FileNotFoundError:
            return Response("oi mate, that emoji doesnt exist or you mispelled it or something",reply=True,delete_after=20)
        return None

    async def cmd_slap(self, author, tagged=None, channel='catalogue'):
        """
        Usage:
            {command_prefx}slap [tagged]
        Spam chat with random shit
        I still can't believe no one bothered to do this before.
        """
        if tagged is None:
            ...
            try:
                r = constants.IMAGE_PATH + "slap" + ".png"
                await self.send_file(channel, r)
            except FileNotFoundError:
                return Response("oi mate, that emoji doesnt exist or you mispelled it or something", reply=True, delete_after=20)
            if author.id == tagged[3:len(tagged)-1]:
                return Response("Errrr did you just slap yourself?")
            else:
                return Response("<@" + "%s" % author.id + ">" " slapped %s" % tagged + ", bad boy!")

    async def cmd_slam(self, author, tagged=None, channel='catalogue'):
        """
        Usage:
            {command_prefx}slam [tagged]
        Spam chat with random shit
        I still can't believe no one bothered to do this before.
        """
        if tagged is None:
            ...
            if tagged.startswith('<@!'):
                id = tagged[3:len(tagged)-1]
            else:
                id = tagged[2:len(tagged)-1]
            ...
            # Download avatar
            try:
                async with self.aiosession.get(link) as reps:
                    with open(constants.EDIT_IMAGE, 'wb') as f:
                        test = await reps.read()
                        f.write(test)
            except urllib.request.URLError as err:
                os.remove(constants.EDIT_IMAGE)
                return Response("I can't download this, but here's the error: " +
                                str(err.reason) + ".Please forgib :(", reply=True, delete_after=20)
            ...
            # Send image
            try:
                r = constants.SLAM_IMAGE
                await self.send_file(channel, r)
            except FileNotFoundError:
                return Response("oi mate, that emoji doesnt exist or you mispelled it or something", reply=True, delete_after=20)

            # Remove file
            os.remove(constants.SLAM_IMAGE)
            os.remove(constants.EDIT_IMAGE)

            if author.id == tagged[3:len(tagged)-1]:
                return Response("Errrr did you just slam yourself?")
            else:
                return Response("<@" + "%s" % author.id + ">" " slammed %s" % tagged + " to the ground ._.")
